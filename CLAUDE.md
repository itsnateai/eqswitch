# EQSwitch v3.3.1 — Claude Code Context
...
| **AutoLoginManager.cs** (376 lines) | DPAPI-encrypted auto-login | Launches EQ and auto-types credentials via `SendInput` on a background thread. Full enter-world automation: username → password → server select → character slot. |
...
| **SettingsForm.cs** (~1780 lines) | 7-tab dark settings GUI | Tabs: General, Hotkeys, Layout, PiP, Paths, Auto-Login, Video. Medieval purple theme. Video tab writes to eqclient.ini (own Save button), other tabs write to AppConfig JSON. |
...
Credentials are encrypted with Windows DPAPI (`DataProtectionScope.CurrentUser`). The encrypted blob is stored in `eqswitch-config.json` as base64. Only the same Windows user account on the same machine can decrypt. The login sequence uses `SendInput` (not clipboard) to type credentials, running on a background thread to avoid blocking the UI.
...
- **Injection requires matching architecture** — Both EQSwitch.exe and eqswitch-hook.dll must be x64 (or both x86). The hook DLL ships pre-built as x64.
...
eqswitch/
  ...
  Native/
    ...
    eqswitch-hook.dll            # Pre-built x64 hook DLL
  ...
  UI/
    ...
    SettingsForm.cs              # 7-tab settings GUI (~1780 lines, Video tab writes eqclient.ini)
...
32-agent audit campaign: ~30 bugs fixed across 11 files. Config defaults baselined to EQ's eqclient.ini. INI section targeting corrected ([Options] vs [Defaults] vs [KeyMaps]). ConfiguredKeys sentinel tracking fixed. GDI Dispose coverage completed for all forms. See CHANGELOG.md for history.
