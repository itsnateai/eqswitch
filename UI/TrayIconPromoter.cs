...
/// Taskbar → Other system tray icons. For a tray-only app like EQSwitch that
...
/// dead paths, accumulating "N EQSwitches" cruft in the Settings list.
///
...
internal static class TrayIconPromoter
{
    ...
    /// "N EQSwitches in tray-overflow Settings" cruft that .NET single-file
        ...
        try
        {
            ...
            foreach (var subName in subNames)
            {
                try
                {
                    ...
                    FileLogger.Info($"TrayIconPromoter.SweepStaleEntries: removed {subName} -> {path}");
                    swept++;
                }
                catch (Exception ex)
                {
                    FileLogger.Warn($"TrayIconPromoter.SweepStaleEntries: subkey {subName}: {ex.Message}");
                }
            }
        }
        catch (Exception ex)
        {
            FileLogger.Warn($"TrayIconPromoter.SweepStaleEntries: {ex.Message}");
        }

        if (swept > 0)
            FileLogger.Info($"TrayIconPromoter.SweepStaleEntries: removed {swept} stale entr{(swept == 1 ? "y" : "ies")} for {ourExeName}");
        return swept;
    }
    ...
    internal static HashSet<string>? CaptureBaseline()
    {
        ...
        {
            FileLogger.Warn($"TrayIconPromoter.CaptureBaseline: {ex.Message}");
            return null;
        }
    }
    ...
    internal static bool TryPromote(string exePath, HashSet<string>? baseline)
    {
            ...
            foreach (var subName in root.GetSubKeyNames())
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(path))
                    {
                        ...
                        {
                            FileLogger.Info($"TrayIconPromoter: {subName} IsPromoted=0 -- respecting user's choice");
                            continue;
                        }
                        if (current is int i1 && i1 == 1) continue;

                        sub.SetValue("IsPromoted", 1, RegistryValueKind.DWord);
                        FileLogger.Info($"TrayIconPromoter: promoted {subName} for {path}");
                        continue;
                    }
                ...
                {
                    FileLogger.Warn($"TrayIconPromoter: subkey {subName}: {ex.Message}");
                }
            }
            ...
            if (!identified && orphanCandidates.Count == 1)
            {
                ...
                try
                {
                    ...
                    if (sub is not null)
                    {
                        ...
                        if (current is int i0 && i0 == 0)
                        {
                            FileLogger.Info($"TrayIconPromoter: orphan {subName} IsPromoted=0 -- respecting user's choice");
                            identified = true;
                        }
                        else
                        {
                            ...
                            sub.SetValue("IsPromoted", 1, RegistryValueKind.DWord);
                            FileLogger.Info($"TrayIconPromoter: claimed orphan {subName} -> wrote ExecutablePath + IsPromoted=1 for {exePath}");
                            identified = true;
                        }
                    }
                }
                ...
                {
                    FileLogger.Warn($"TrayIconPromoter: claim orphan {subName}: {ex.Message}");
                }
            }
            else if (orphanCandidates.Count > 1)
            {
                FileLogger.Info($"TrayIconPromoter: {orphanCandidates.Count} new orphan subkeys -- deferring to next tick");
            }
        ...
        catch (Exception ex)
        {
            ...
            FileLogger.Warn($"TrayIconPromoter: {ex.Message}");
            return false;
        }
    }
}
