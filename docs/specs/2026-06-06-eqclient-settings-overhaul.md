...
- [ ] **Phase 1 — Main form onto the table.** Replace `LoadFromIni`/`ApplyToIni` with table loops. Fix `AANoConfirm` polarity (one row). Add loaded-baseline snapshot + per-key-touch save. Remove Bucket-2 keys from `EnforceOverrides` (keep the slim geometry block + Operational). Verify main window display == live INI; toggle round-trips; no ghost in a 2nd section.
- [ ] **Phase 2 — ChatSpam onto the table** (add touch-gating; drop launch enforce).
