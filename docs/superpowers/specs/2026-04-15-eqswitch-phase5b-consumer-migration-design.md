...
3. **Any `resolvedSlot == 0` return now aborts unconditionally** with `abortWrongCharacter = true`. This covers three sub-cases that pre-extraction and the first Phase 5b pass handled differently: (a) named-mode name-miss (pre-extraction and Phase 5b both aborted — unchanged), (b) malformed target with `CharacterSlot == 0 && Name == ""` (pre-extraction silently fell through on default selection — Phase 5b's original pass aborted in named-mode only), (c) slot-mode name-miss (pre-extraction and Phase 5b's original pass both silently fell through on default selection). Case (c) was promoted from a Phase 6 deferral to an in-scope Phase 5b fix during cross-cutting review because auto-login character selection is the key product feature and silent wrong-character is a ship-stopper. Users who need to auto-login while MQ2 is in slot-mode must use an explicit `CharacterSlot > 0` (routes through Decide Case 3 and never reaches this branch).
4. **Name match uses `StringComparison.OrdinalIgnoreCase`**, matching the pre-extraction `CharSelectReader.RequestSelectionByName` contract. The rest of the codebase's name compares (`FindCharacterByName`, `AccountKey.Matches`, `FindAccountByName`) use `Ordinal` because they compare config-to-config data where casing is stable. This function compares config-to-heap where MQ2 scrape artifacts and server renames can introduce casing drift — tolerance is the right default.
...
| `CharacterAliases` out of sync with `LegacyCharacterProfiles` after a manual JSON edit | Wrong priority override applied | Phase 5b's fold-commit added a defense-in-depth resync block to `AppConfig.Validate()` that re-derives `CharacterAliases` from `LegacyCharacterProfiles` when the former is empty. Phase 6 drops this block with the legacy field. |
