...
> ⚠️ **POST-2026-05-15 SUPERSEDED (R3 verifier-pair sweep):** the JoinServer/Cheat-Engine substeps in this handoff are obsolete. JoinServer is now PINNED at eqmain+0x13C30 via emu-branch eqmain.h, and the in-process call wrapper ships as `MQ2Bridge::JoinServerDirect()` in `Native/mq2_bridge.cpp`. See:
>
> - `_.eqswitch-re/mq2-autologin-walkthrough.md` — Verified offsets reference + Section 5.1
> - `_.eqswitch-re/mq2-autologin-eqswitch-diff.md` — Diff 4 design
> - `EQSwitch/Native/eqmain_offsets.h` — `RVA_PINST_LoginServerAPI`, `RVA_FN_LoginServerAPI_JoinServer`, `RVA_VTABLE_LoginServerAPI_Secondary`
> - `_.eqswitch-re/findings.md` SUPERSEDED banner (top of file)
>
> The GiveTime detour itself (the v7 phase-1/2/3/4 work that this handoff describes) IS still relevant and shipping in `Native/login_givetime_detour.cpp`. Only the JoinServer-related Cheat-Engine probes in this handoff are obsolete.
